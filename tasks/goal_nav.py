...
class GoalNavigationTask(BaseTask):
    ...
    def compute_reward(self, task_state, prev_env_state, env_state, transition_info) -> TaskStepResult:
        ...
        if bool(metrics["success"]) and not bool(task_state.get("success_bonus_paid", False)):
            success_bonus = float(self.config["reward_weights"]["goal_nav"].get("success_bonus", 0.0))
        ...
        repeated_penalty = float(weights["repeated_goal"] * repeated_ratio)
        reward = progress_reward + completion_reward + coverage_reward + distance_penalty + repeated_penalty + success_bonus
